...
class CNCMANY_DRAWFILTER_NEGATIVE_net(Classifier):
    """
    Chemical/non-chemical/many-chemical network specialized with drawingfilter (CNC)(negative) network
    Predict whether an image contains a chemical structure or not.
    """

    def __init__(self):
        super().__init__(['manychemical', 'nonchemical', 'onechemical'],
                        "Chemical/non-chemical/many-chemical network specialized with drawingfilter (negative)",
                        "cncmany_drawfilter_negative",
        ...
        try:
            self.model.load_state_dict(torch.load('./models/negative_cncmany_drawingfilter_relative_rank.pth', map_location=torch.device('cpu')))
